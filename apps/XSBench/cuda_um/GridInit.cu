...
SimulationData move_simulation_data_to_device( Inputs in, int mype, SimulationData SD )
{
	...
	GSD.num_nucs = SD.num_nucs;
	gpuErrchk( cudaMemPrefetchAsync(GSD.num_nucs, sz, 0));
	total_sz += sz;
	...
	GSD.concs = SD.concs;
	gpuErrchk( cudaMemPrefetchAsync(GSD.concs, sz, 0));
	total_sz += sz;
	...
	GSD.mats = SD.mats;
	gpuErrchk( cudaMemPrefetchAsync(GSD.mats, sz, 0));
	total_sz += sz;
	...
	GSD.unionized_energy_array = SD.unionized_energy_array;
	gpuErrchk( cudaMemAdvise(GSD.unionized_energy_array, sz, cudaMemAdviseSetAccessedBy, 0));
	gpuErrchk( cudaMemAdvise(GSD.unionized_energy_array, sz, cudaMemAdviseSetPreferredLocation, 0));
	gpuErrchk( cudaMemPrefetchAsync(GSD.unionized_energy_array, sz, 0));
	total_sz += sz;
	...
	GSD.index_grid = SD.index_grid;
	gpuErrchk(cudaMemAdvise(GSD.index_grid, sz, cudaMemAdviseSetReadMostly, 0));
	
	gpuErrchk(cudaMemAdvise(GSD.index_grid, sz, cudaMemAdviseSetAccessedBy, 0));		
	
	//gpuErrchk(cudaMemAdvise(GSD.index_grid, sz, cudaMemAdviseSetPreferredLocation, 0)); 
	gpuErrchk(cudaMemAdvise(GSD.index_grid, sz, cudaMemAdviseSetPreferredLocation, cudaCpuDeviceId));

	//gpuErrchk(cudaMemPrefetchAsync(SD.index_grid, sz, 0));
	
	//gpuErrchk(cudaMemPrefetchAsync(SD.index_grid, GSD.length_index_grid*3/4*sizeof(int), 0));
	//gpuErrchk(cudaMemAdvise(GSD.index_grid, GSD.length_index_grid*3/4*sizeof(int), cudaMemAdviseSetPreferredLocation, 0)); 
	//gpuErrchk(cudaMemAdvise((char*)GSD.index_grid+GSD.length_index_grid*3/4*sizeof(int), GSD.length_index_grid*1/4*sizeof(int),
	...
	GSD.nuclide_grid = SD.nuclide_grid;
	gpuErrchk( cudaMemAdvise(GSD.nuclide_grid, sz, cudaMemAdviseSetAccessedBy, 0));
	gpuErrchk( cudaMemAdvise(GSD.nuclide_grid, sz, cudaMemAdviseSetPreferredLocation, 0));
	gpuErrchk( cudaMemPrefetchAsync(GSD.nuclide_grid, sz, 0));
	total_sz += sz;
...
SimulationData grid_init_do_not_profile( Inputs in, int mype )
{
	...
	int num_devices = 0, attr_val=0;
	cudaGetDeviceCount( &num_devices );
	cudaSetDevice(0);
	cudaDeviceGetAttribute ( &attr_val,  cudaDevAttrConcurrentManagedAccess, 0 );
	printf("Found %d GPUs, set to use device %d\n", num_devices, 0);
	printf("device 0 has cudaDevAttrConcurrentManagedAccess. = %d\n", attr_val);
	...
	gpuErrchk( cudaMallocManaged((void **)&SD.nuclide_grid,  SD.length_nuclide_grid * sizeof(NuclideGridPoint), cudaMemAttachGlobal));
	printf("after allocating SD.nuclide_grid %p\n", SD.nuclide_grid);
	...
	*/
	...
	if( in.grid_type == UNIONIZED )
	{
		...
		gpuErrchk( cudaMallocManaged((void **)&SD.unionized_energy_array, SD.length_unionized_energy_array * sizeof(double), cudaMemAttachGlobal));
		printf("after allocating SD.unionized_energy_array %p\n", SD.unionized_energy_array);
		...
		gpuErrchk( cudaMallocManaged((void **)&SD.index_grid, SD.length_index_grid * sizeof(int), cudaMemAttachGlobal));
		printf("after allocating SD.index_grid %p\n", SD.index_grid);
