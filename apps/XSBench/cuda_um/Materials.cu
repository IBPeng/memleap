...
int * load_num_nucs(long n_isotopes)
{
  ...
  gpuErrchk( cudaMallocManaged((void**)&num_nucs, 12*sizeof(int), cudaMemAttachGlobal));
  printf("after allocating num_nucs 0x%p\n", num_nucs);
...
int * load_mats( int * num_nucs, long n_isotopes, int * max_num_nucs )
{
	*max_num_nucs = 0;
	...
	gpuErrchk( cudaMallocManaged((void **) &mats, num_mats * (*max_num_nucs) * sizeof(int) ,cudaMemAttachGlobal));
	printf("after allocating mats 0x%p\n", mats);
...
double * load_concs( int * num_nucs, int max_num_nucs )
{
	...
	gpuErrchk( cudaMallocManaged((void **) &concs, 12 * max_num_nucs * sizeof( double ),cudaMemAttachGlobal));
	printf("after allocating mats 0x%p\n", concs);
