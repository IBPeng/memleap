...
void BFSGraph( int argc, char** argv) 
{
  ...
  uvm_profiling_init();
	...
	*/
	...
	checkRes(cudaMallocManaged((void**) &h_graph_nodes,  sizeof(Node)*no_of_nodes));
...
#ifdef CUPTI_PROFILING
	um_dataobj_map.emplace_back( h_graph_nodes, sizeof(Node)*no_of_nodes, "h_graph_nodes");
	um_dataobj_map.emplace_back( h_graph_mask, sizeof(bool)*no_of_nodes, "h_graph_mask");
	um_dataobj_map.emplace_back( h_updating_graph_mask, sizeof(bool)*no_of_nodes, "h_updating_graph_mask");
	um_dataobj_map.emplace_back( h_graph_visited, sizeof(bool)*no_of_nodes, "h_graph_visited");
#endif
...
#ifdef CUPTI_PROFILING
	um_dataobj_map.emplace_back( h_graph_edges, sizeof(int)*edge_list_size, "h_graph_edges");
#endif
...
#ifdef CUPTI_PROFILING
	um_dataobj_map.emplace_back( h_cost, sizeof(int)*no_of_nodes, "h_cost");
#endif
...
#ifdef CUPTI_PROFILING
	um_dataobj_map.emplace_back( d_over, sizeof(bool), "d_over");       
#endif
  ...
  high_resolution_clock::time_point time_st = high_resolution_clock::now();
	...
	do
	{
		...
		checkRes(cudaDeviceSynchronize());
		k++;
	}
	while(d_over[0]);
  ...
  duration<double> time_span = duration_cast<duration<double>>(time_end - time_st);
  printf("Kernel Executed %d times for %.3f seconds\n",k, time_span.count());
